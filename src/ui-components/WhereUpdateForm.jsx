...
export default function WhereUpdateForm(props) {
  const {
    id: idProp,
    where: whereModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [whereRecord, setWhereRecord] = React.useState(whereModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Where, idProp)
        : whereModelProp;
      setWhereRecord(record);
    };
    queryData();
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Where.copyOf(whereRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WhereUpdateForm")}
      {...rest}
    >
      <TextField
        label="Business name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || whereModelProp)}
